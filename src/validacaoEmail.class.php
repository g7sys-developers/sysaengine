...
class validacaoEmail
{
     ...
     * return           string
     */
    public function validarEmail(string $token, string $email, int $id): string
    {
        ...
        $mensagem_token_expirado = $this->layoutMensagem($row['mensagem_token_expirado'], $row);
        $mensagem_sucesso = $this->layoutMensagem($row['mensagem_sucesso'], $row);
        if (!$row['valido'])
     ...
     * return           string
     */
    public function layoutMensagem(string $mensagem, $dados): string
    {
        ...
        $vars = [
            ...
            'codemp' => $dados['codemp'],
            'url_validacao' => sysa::getUrlBase() . '/validarEmail.php?token=' . $dados['validacao_token'] . '&email=' . $dados['email'] . '&id=' . $dados['id_validacao_email'] . '&codigo_usuario=' . $dados['codigo_usuario'],
    ...
    public function requerer(string $titulo_email, string $mensagem, string $email, int $codigo_usuario, string $mensagem_token_expirado, int $codemp, string $mensagem_sucesso): int
    {
        ...
        $titulo_email = $this->layoutMensagem($titulo_email, $row);
        $mensagem = $this->layoutMensagem($mensagem, $row);
